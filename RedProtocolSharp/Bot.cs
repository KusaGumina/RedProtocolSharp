...
public class Bot
{
    ...
    #endregion
    #region MessageQueue
    public class MessageReceiveQueue : EventArgs
    {
        public delegate void MessageReceiveHandler(MsgType.Payload payload);
        public event MessageReceiveHandler OnMessageReceived;
        public void ReceiveMessage(object? sender,MessageEventArgs e,Bot bot)
        {
            var payload = bot.parseMsg(e.Data);
            OnMessageReceived?.Invoke(payload);
        }
    }
    public MessageReceiveQueue MessageReceive = new();
